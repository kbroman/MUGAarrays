...
input <- "gigamuga_untrimmed.fa"
output_dir <- "../results_gm_untrimmed"
...
probes <- qtl2::read_csv("../../Sequences/gm_untrimmed_seq.csv")
...
if(length(todrop) > 0) results <- results[-todrop,]

saveRDS(results, paste0(output_dir, "/gm_untrimmed_blastn_results.rds"), compress=FALSE)
