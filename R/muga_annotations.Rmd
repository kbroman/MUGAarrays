...
```{r read_muga_data, echo=FALSE, results="hide"}
load("../UNC/snps.muga.Rdata")
muga_unc <- snps
muga_unc$chr <- sub("^chr", "", muga_unc$chr)
```

This contains information for `r nrow(muga_unc)` SNPs. All are on
...
sequence. (There are `r sum(!is.na(muga_unc$seq.B))` of these.)

The probes are again 50 bp long. Whereas before it seemed like in the
cases with a second probe sequence the probe sequences contained the
SNP allele, here that doesn't seem to be the case.
