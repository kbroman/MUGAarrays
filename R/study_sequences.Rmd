...
author: "Karl Broman"
date:   "2018-11-02 (corrected slightly `r Sys.Date()`)"
output:
    html_document:
        code_folding: hide
        toc: true
...
whether `IlmnStrand == SourceStrand` or not, but that there are
`r sum(gm_snp_strand_problem)` mismatches on the GigaMUGA and
`r sum(mm_snp_strand_problem)` mismatches on the MegaMUGA. But these are
