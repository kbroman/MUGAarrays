...
The annotation files we created are at
<https://github.com/kbroman/MUGAarrays/blob/master/UWisc> (change this).

## Preliminaries

We will first load some R packages.
...
We will now load MUGA annotation file from UNC, and
...
muga_blast <- readRDS("../Blast/results_muga_bigzips/muga_blastn_results.rds")
...
In the blast results, We find that `r add_commas(sum(muga_nhits==1))`
...
perfect match in the mm39 mouse genome assembly. These markers are on
...
muga_pos <- setNames(paste(muga_uwisc$chr, muga_uwisc$bp_mm39, sep=":"), muga_uwisc$marker)[muga_uwisc$unique]
...
write.table(muga_uwisc[!is.na(muga_uwisc$chr) & muga_uwisc$chr %in% c(1:19,"X"), c("chr", "bp_mm39")],
...
muga_uwisc_conv <-  muga_uwisc[,c("chr","bp_mm39","marker")]
names(muga_uwisc_conv) <- c("chr","pos","marker")
muga_cox <- mmconvert(muga_uwisc_conv)

#pruning out NA, M and PAR, Y
muga_uwisc_pr <- muga_uwisc[muga_uwisc$chr != "Y" & muga_uwisc$chr != "PAR" & muga_uwisc$chr != "M" & !is.na(muga_uwisc$chr),]

#sort by marker
muga_uwisc_pr <- muga_uwisc_pr[muga_cox$marker,]

# verify stuff
stopifnot( all(muga_cox[,1] == muga_uwisc_pr$marker) )
stopifnot( all(muga_cox[,2] == muga_uwisc_pr$chr) )
stopifnot( all(muga_cox[,6] ==  muga_uwisc_pr$bp_mm39) )
#stopifnot( all(muga_g2f1[,1] == muga_uwisc$chr[1:nrow(muga_g2f1)]) )
#stopifnot( all(muga_g2f1[,2] == muga_uwisc$bp_mm39[1:nrow(muga_g2f1)]) )
```
...
#reorder chrs in muga_wisc to be 1:19, X , Y, M, PAR, NA
muga_uwisc$chr <- factor(muga_uwisc$chr,levels=ord)
muga_uwisco <- muga_uwisc[order(muga_uwisc$chr),]

#redorder chrs in muga_cos to be 1:19, X
#muga_cox$chr <- factor(muga_cox$chr,levels=ord)

nas <- rep(NA, nrow(muga_uwisc)-nrow(muga_cox))
muga_uwisc.2 <- cbind(muga_uwisco,
...
muga_uwisc.3 <- muga_uwisc.2[,cols_new.2]

# write mugaMUGA file
muga_file <- "../UWisc/muga_uwisc_v1.csv"
write.table(muga_uwisc.3, muga_file, sep=",", quote=FALSE,
            row.names=FALSE, col.names=TRUE)
```
