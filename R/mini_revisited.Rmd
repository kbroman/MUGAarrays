...
My goal here was to compare the two sets of annotations. As we will
see, the UNC annotation file includes many additional markers. And so
...
- `r exec_summ$no_hit` markers with no perfect blast hit in the mouse genome (assembly
  mm10, GRCm38). This is on top of the `r exec_summ$transgene` transgene markers

- `r exec_summ$n_diff_chr` markers where the UNC annotation file has them on a different
...
- The remaining `r broman::add_commas(exec_summ$n_matching_chr)`
  markers having matching chromosome IDs between the UNC annotation
  file and the blast results,
  but `r exec_summ$n_pos_mismatch` of them have discrepancies in their position.
  Of these, `r exec_summ$n_pos_mismatch_old` were on the previous verion of the
  array and are on the mitochondria, while the other
  `r exec_summ$n_pos_mismatch - exec_summ$n_pos_mismatch_old` are new.
...
these to the X chromsome. We still see these, and we'll force them
back to `PAR`.
...
`r sum(!is.na(mini_uwisc$chr) & mini_uwisc$chr != "PAR" & mini_unc$pos != mini_uwisc$pos)`
...
The `r sum(result$unc_chr=="M")` mitchondrial markers were
...
exec_summ <- list(new_markers=nrow(mini_unc)-nrow(uw),
                  ...
                  n_matching_chr=sum(!is.na(mini_uwisc$chr) & mini_uwisc$chr != "PAR" & mini_unc$chr == mini_uwisc$chr),
