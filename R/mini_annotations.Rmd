...
file](https://github.com/kbroman/MUGAarrays/blob/minimuga/UNC/miniMUGA-Marker-Annotations.csv)
...
second probe sequence, and in those cases it seems again that the two
...
**I've not done this part yet, so the code is suppressed.**
...
#for(chr in c(1:19,"X")) {
#    # grab positions
#    mini_g2f1_bp <- mini_g2f1[mini_g2f1[,1]==chr,2]
#    mini_g2f1_cM <- mini_g2f1[mini_g2f1[,1]==chr,5]
#
#    # Overall cM:Mb ratio
#    ratio <- diff(range(mini_g2f1_cM))/(diff(range(mini_g2f1_bp))/1e6)
#
#    # shift to make 3 Mbp == 0 cM
#    delta <- -min(mini_g2f1_cM) + ratio * (min(mini_g2f1_bp)/1e6 - 3)
#
#    # do the shift
#    mini_g2f1[mini_g2f1[,1]==chr,5] <- round(mini_g2f1_cM + delta, 3)
#}
```
